...
(defn breeds-view []
  (r/with-let [_ (rf/dispatch [:fetch-breeds])]
       ...
       (if loading?
         ...
         [:ul
          (for [breed breeds]
            ...
            [:li [:a {:href "#"
                      :on-click #(rf/dispatch [:set-view :breed-images-view breed])}
                  (str (:sub-breed breed) " " (:primary-breed breed))]])])])))

(defn breed-images-view []
  (let [breed @(rf/subscribe [:images/current-breed])]
    (r/with-let [_ (rf/dispatch [:fetch-breed-images breed])]
      (let [images @(rf/subscribe [:breed-images breed])
            paged-images @(rf/subscribe [:images/paged breed])
        ...
        (if (or (nil? loading?) loading?)
          ...
          [:div
           ...
           [:button {:on-click #(rf/dispatch [:set-view :breeds-view])}
            "<- Back to breeds"]
